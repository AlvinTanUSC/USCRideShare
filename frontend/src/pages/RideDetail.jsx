      ...
      <div className="container mx-auto px-4 py-8 max-w-3xl">
        ...
        <div className="bg-white rounded-lg shadow-md p-8">
          ...
          <div className="flex justify-between items-start mb-6">
            ...
            <span className={`px-4 py-2 rounded-full text-sm font-semibold ${getStatusBadgeColor(ride.status)}`}>
              {ride.status}
            </span>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            <div>
              ...
              <p className="text-lg text-gray-900">{ride.destination.replace('_', ' ')}</p>
            </div>

            <div>
              <h3 className="text-sm font-medium text-gray-500 mb-1">Departure Time</h3>
              <p className="text-lg text-gray-900">{formatDateTime(ride.departureDatetime)}</p>
            </div>

            {ride.flexibleTime && (
              <div>
                <h3 className="text-sm font-medium text-gray-500 mb-1">Time Flexibility</h3>
                <p className="text-lg text-gray-900">± {ride.timeFlexibilityMinutes} minutes</p>
              </div>
            )}

            <div>
              <h3 className="text-sm font-medium text-gray-500 mb-1">Max Passengers</h3>
              <p className="text-lg text-gray-900">{ride.maxPassengers}</p>
            </div>

            <div>
              <h3 className="text-sm font-medium text-gray-500 mb-1">Cost Split</h3>
          ...
          <div className="border-t pt-6 mt-6">
            <p className="text-sm text-gray-500">
              Created on {new Date(ride.createdAt).toLocaleDateString('en-US', {
                year: 'numeric',
                month: 'long',
                day: 'numeric',
              })}
